    ...
    private Element xmlAssembler_addAttributes(Element root) {
        ...
        try {
            ...
            for (AttributeSchemaDef att : env.Parameters.AttributeSchemas.getAttributes()) {
                ...
                // isCode for UMLS node
                Element attIsUMLS = new Element("isCode");
                if( att.isCode )
                    attIsUMLS.setText( "true" );
                else
                    attIsUMLS.setText( "false" );
                attribute.addContent(attIsUMLS);
    ...
    public Element XMLassembler_addClassAttributes(
        ...
        if( ( thisclass.privateAttributes != null ) &&
            ...
            for (AttributeSchemaDef att : thisclass.getAttributes() ) {
                ...
                Element attIsCode = new Element("isCode");
                if( att.isCode )
                    attIsCode.setText("true");
                else
                    attIsCode.setText("false");
                attribute.addContent(attIsCode);
