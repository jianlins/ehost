    ...
    public Vector<iListable> getAttributesForShow() {
        ...
        if (attributes != null) {
            ...
            for (AnnotationAttributeDef attribute : attributes) {
                ...
                boolean accountedFor = false;
                for (AttributeSchemaDef attributedef : currentClass.privateAttributes) {
                    if (attributedef.getName().equals(attribute.name)) {
                        accountedFor = true;
                    }
                }

                for (AttributeSchemaDef attributedef : env.Parameters.AttributeSchemas.getAttributes()) {
                    if (attributedef.getName().equals( attribute.name)) {
                        accountedFor = true;
                    }
                }
                ...
                if (!accountedFor) {
                    finalList.add(new AttributeSchemaDef( attribute.name, attribute.value));
                }
            }
        }
