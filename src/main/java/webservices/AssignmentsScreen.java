 ...
 * @author Yarden
 */
public class AssignmentsScreen extends JPanel implements AnnotationAdminComMgrEventListener {
...
//        });
    ...
    public void sync(){
        try {
            ...
            if(Parameters.WorkSpace.CurrentProject!=null)
            {
            	gui.verifyAnnotationsWereSaved();
                try
                {
        	        String[] names = Parameters.WorkSpace.CurrentProject.list();
        	        for(String name : names)
        	        {
        	        	File file = new File(Parameters.WorkSpace.CurrentProject.getAbsolutePath() +  File.separator + name);
